...
class Game:
    def __init__(self):
        ...
        try:
            ...
            print(f"Window's actual problem is: {e}")
    ...
    def load_level(self, map_id):
        ...
        for spawner in self.tilemap.extract([('spawners', 0), ('spawners', 1)]):
            if spawner['variant'] == 0:
                self.player.pos.x = spawner['pos'][0]
                self.player.pos.y = spawner['pos'][1]
                self.player.air_time = 0
            else:
                self.enemies.append(Enemy(self, (spawner['pos'][0], spawner['pos'][1]), (8,15)))
    ...
    def run(self):
        ...
        while True:
            ...
            for proj in self.projectiles.copy():
                ...
                self.display.blit(img, (proj[0].x - img.get_width() / 2 - render_scroll.x, proj[0].y - img.get_height()/ 2 - render_scroll.y))
                if self.tilemap.solid_check(proj[0]):
                    self.projectiles.remove(proj)
                    for i in range(4):
                        self.sparks.append(Spark(proj[0], random.random() - 0.5 + (math.pi if proj[1] > 0 else 0), 2 + random.random()))
                elif proj[2] > 360:
                    self.projectiles.remove(proj)
                elif abs(self.player.dashing) < 50:
                    if self.player.rect().collidepoint(proj[0]):
                        self.projectiles.remove(proj)
                        self.dead +=1
                        self.sfx['hit'].play()
            ...
            display_sillho = display_mask.to_surface(setcolor=(0,0,0,180), unsetcolor=(0,0,0,0))
            for o in [(-1,0), (1,0), (0,-1), (0,1)]:
                self.display2.blit(display_sillho, o)
